...
void ArrowAnalyzer::setFirstArrows(int id)
{
    ...
    if (mField->getShipTypesNum().first() != 0 ||
            (mField->getShipTypesNum().first() != 0
             && (mField->getShipTypesNum().at(1) != 0 || mField->getShipTypesNum().at(2) != 0
                 || mField->getShipTypesNum().at(3) != 0))){
    ...
    if((mField->getShipTypesNum().at(1) != 0 || mField->getShipTypesNum().at(2) != 0
        || mField->getShipTypesNum().at(3) != 0) && orientation.last()){
...
void ArrowAnalyzer::analyzeNextArrow(Orientation::Orient orient, ImageID iD, int id)
{
	...
	case 1:
        ...
        if (mField->getShipTypesNum().at(1) != 0 && isEmptyAround(point) && orientation.at(int(orient))){
			mFieldView->addImage(id, ARROW_IN_ITSELF);
			cellsWithArrow.append(id);
		}
        if (((mField->getShipTypesNum().at(2) != 0) || (mField->getShipTypesNum().at(3) != 0))
				&& isEmptyAround(point) && orientation.at(int(orient))){
            if (mField->getShipTypesNum().at(2) != 0){
            ...
            }else if (mField->getShipTypesNum().at(3) != 0){
				if (isLeftOrRight && orientation.at(int(orient))
	...
	case 2:
		mFieldView->addImage(firstId, FIRT_POINT);
		cellsWithArrow.append(firstId);
        if (mField->getShipTypesNum().at(2) != 0 && isEmptyAround(point) && orientation.at(int(orient))){
			mFieldView->addImage(id, ARROW_IN_ITSELF);
			cellsWithArrow.append(id);
		}
	...
	case 3:
		mFieldView->addImage(firstId, FIRT_POINT);
		cellsWithArrow.append(firstId);
        if (isEmptyAround(point) && mField->getShipTypesNum().at(3) != 0 && orientation.at(int(orient))){
			mFieldView->addImage(id, ARROW_IN_ITSELF);
			cellsWithArrow.append(id);
		}
		break;
...
void ArrowAnalyzer::setArrow(int id, ImageID iD, int difference, Orientation::Orient orient)
{
	...
	if (orientation[int(orient)]){
        ...
        if (mField->getShipTypesNum().at(qAbs(difference)) != 0 && checkCoord(point.first, point.second) && orientation[int(orient)]){
			int coefficient = 1;
			point = coordinates(id);
